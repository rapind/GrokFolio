class Admin::ProjectsController < ApplicationController
  ...
  def destroy
    destroy! do |success, failure|
      success.js { 
        flash[:notice] = ''
        render :json => {:title => 'Success', :message => "Successfuly removed the project."} 
      }
      failure.js { render :json => {:title => 'Error', :message => 'Ran into an error removing the project. Please try again later.'} }
    end
  end
  ...
  def update_position
    begin
      project = Project.find(params[:id]) # grab the object
      project.insert_at(params[:position].to_i) # update the object's order
  ...
  private #-------
    ...
    def collection
      @projects ||= end_of_association_chain.find :all, :order => 'position'
    end
